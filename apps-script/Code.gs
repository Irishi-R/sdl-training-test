...
function doPost(e) {
  try {
    ...
    if (data.action === 'saveResponse')    return out(saveResponse(data));
...
function getCourse(courseId) {
  ...
  for (var i = 1; i < rows.length; i++) {
    ...
    for (var q = 0; q < 4; q++) {
      var pOff = 4  + q * 3; // pre  questions start at col 4
      var rOff = 16 + q * 3; // post questions start at col 16
...
function saveResponse(data) {
  ...
  if (sheet.getLastRow() === 0) {
    sheet.appendRow([
      'Submission ID', 'Timestamp', 'Student ID', 'Name', 'School',
...
function setupDemoData() {
  ...
  // Each row: 4 base cols + 4 pre questions × 3 cols + 4 post questions × 3 cols = 28 cols
  var courses = [
    ...
    ['C005','Technical Skills','Machine Operation Safety','https://www.youtube.com/watch?v=VIDEO_ID_HERE',
     ...
     'What should you do if you notice a machine malfunction during operation?','mcq',
  ...
  s3.clearContents();
  s3.getRange(1, 1, 1, 16).setValues([[
    'Submission ID', 'Timestamp', 'Student ID', 'Name', 'School',
    'Course ID', 'Course Name', 'Category',
    'Pre Q1', 'Pre Q2', 'Pre Q3', 'Pre Q4',
    'Post Q1', 'Post Q2', 'Post Q3', 'Post Q4'
  ...
  Logger.log('Courses: C001 Fire Safety | C002 First Aid Basics | C003 Communication Skills | C004 Teamwork Essentials | C005 Machine Operation Safety');
